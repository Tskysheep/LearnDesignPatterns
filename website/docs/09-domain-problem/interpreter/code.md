...
sidebar_position: 2
title: 源代码
description: Interpreter 模式的完整实现代码
---
...
```cpp title="Interpreter.h" showLineNumbers
...
## 实现文件（Interpreter.cpp）

```cpp title="Interpreter.cpp" showLineNumbers
...
add_executable(Pattern_Interpreter Interpreter.cpp Interpreter.h)
```

:::tip 编译运行
```bash
cd build
cmake --build . --target Pattern_Interpreter
./Pattern_Interpreter
```
:::
