...
sidebar_position: 2
title: 源代码
description: TemplateMethod 模式的完整实现代码
---

# 源代码

## 头文件（TemplateMethod.h）

```cpp title="TemplateMethod.h" showLineNumbers
...
## 实现文件（TemplateMethod.cpp）

```cpp title="TemplateMethod.cpp" showLineNumbers
...
add_executable(Pattern_TemplateMethod TemplateMethod.cpp TemplateMethod.h)
target_compile_features(Pattern_TemplateMethod PRIVATE cxx_std_17)
```

:::tip 编译运行
```bash
cd build
cmake --build . --target Pattern_TemplateMethod
./Pattern_TemplateMethod  # or .\Release\Pattern_TemplateMethod.exe on Windows
```
:::
