...
sidebar_position: 2
title: 源代码
description: Strategy 模式的完整实现代码
---
...
```cpp title="Strategy.h" showLineNumbers
#ifndef STRATEGY_H
#define STRATEGY_H
...
```cpp title="Strategy.cpp" showLineNumbers
#include "Strategy.h"
#include <iostream>
#include <iomanip>
#include <sstream>
...
add_executable(Pattern_Strategy Strategy.cpp Strategy.h)
target_compile_features(Pattern_Strategy PRIVATE cxx_std_17)
```

:::tip 编译运行
```bash
cd build
cmake --build . --target Pattern_Strategy
./Pattern_Strategy  # or .\Release\Pattern_Strategy.exe on Windows
```
:::
