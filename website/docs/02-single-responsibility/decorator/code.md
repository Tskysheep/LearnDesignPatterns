...
sidebar_position: 2
title: 源代码
description: Decorator 模式的完整实现代码
---
...
```cpp title="Decorator.h" showLineNumbers
#ifndef DECORATOR_H
#define DECORATOR_H
...
```cpp title="Decorator.cpp" showLineNumbers
#include "Decorator.h"
#include <iostream>
#include <algorithm>
#include <sstream>
...
add_executable(Pattern_Decorator Decorator.cpp Decorator.h)
target_compile_features(Pattern_Decorator PRIVATE cxx_std_17)
```

:::tip 编译运行
```bash
cd build
cmake --build . --target Pattern_Decorator
./Pattern_Decorator
```
:::
