...
```cpp title="Proxy.h" showLineNumbers
#pragma once
#include <string>
#include <memory>
#include <iostream>
...
```cpp title="Proxy.cpp" showLineNumbers
#include "Proxy.h"
#include <iostream>
#include <string>
#include <vector>
...
add_executable(Pattern_Proxy Proxy.cpp Proxy.h)
target_compile_features(Pattern_Proxy PRIVATE cxx_std_17)
```

:::tip 编译运行
```bash
cd build
cmake --build . --target Pattern_Proxy
./Pattern_Proxy
```
:::
