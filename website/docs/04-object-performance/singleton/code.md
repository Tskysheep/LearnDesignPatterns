...
sidebar_position: 2
title: 源代码
description: Singleton 模式的完整实现代码
---
...
```cpp title="Singleton.h" showLineNumbers
#pragma once
...
```cpp title="Singleton.cpp" showLineNumbers
#include "Singleton.h"
#include <thread>
#include <sstream>
#include <algorithm>
...
add_executable(Pattern_Singleton Singleton.cpp Singleton.h)
```

:::tip 编译运行
```bash
cd build
cmake --build . --target Pattern_Singleton
./Pattern_Singleton
```
:::